...
function insertComment($videoId, $userId, $userType, $commentText) {
    ...
    if ($stmt_insert_comment->execute()) {
        // Fetch the comment details to return
        ...
        if ($result->num_rows > 0) {
            ...
            // Return the comment data as an array
            return [
                'status' => 'success',
...
function getComments($videoId) {
    ...
    return $stmt_comments->get_result(); // Return result directly to be processed by the caller
}
...
function verifyAndChangePassword($table, $email, $recent_password, $new_password) {
    ...
    if ($result->num_rows > 0) {
        // User is found, proceed with password update
...
// Function to fetch the latest comment details for a specific video
...
function fetchAllCrops() {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            // Assuming the images are stored in ../asset/images/ folder
            $row['image'] = '../asset/images/' . basename($row['image']);
            $crops[] = $row;
        }
    }
    $conn->close();
...
function getStudentNameByEmail($email) {
    ...
    $sql = "SELECT first_name, last_name FROM student WHERE email = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $email); 
    $stmt->execute();
    $result = $stmt->get_result();
...
function getFarmerNameByEmail($email) {
    $conn = getConnection();
    $sql = "SELECT first_name, last_name FROM farmer WHERE email = '$email'";
